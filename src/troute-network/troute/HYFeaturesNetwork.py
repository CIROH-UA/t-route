...
from troute.nhd_network import (
    reverse_dict,
    extract_connections,
    reverse_network,
    reachable,
)
from .rfc_lake_gage_crosswalk import (
    get_rfc_lake_gage_crosswalk,
...
def read_geopkg(file_path, compute_parameters, waterbody_parameters, cpu_pool):
    ...
    matched_layers = {
        key: find_layer_name(available_layers, pattern)
    ...
    if any(
        [
            data_assimilation_parameters.get("streamflow_da", {}).get(
    ...
    if (
        hybrid_parameters.get("run_hybrid_routing", False)
        and "nexus" not in layers_to_read
    ):
    ...
    if cpu_pool > 1:
        ...
        table_dict = {
            layers_to_read[i]: gpkg_list[i] for i in range(len(layers_to_read))
        }
    else:
        table_dict = {
            layer: read_layer(matched_layers[layer]) for layer in layers_to_read
        }
    ...
    if (
        "link" in flowpath_attributes_df.columns
        and "id" not in flowpath_attributes_df.columns
    ):
    ...
    if not flowpath_attributes_df.empty and not flowpaths_df.empty:
        ...
        unique_cols = set(flowpath_attributes_df.columns).difference(
            set(flowpaths_df.columns)
        )
        unique_cols.add("id")
...
def read_ngen_waterbody_df(parm_file, lake_index_field="wb-id", lake_id_mask=None):
    ...
    if Path(parm_file).suffix == ".gpkg":
        ...
        df = df.drop(
            ["id", "toid", "hl_id", "hl_reference", "hl_uri", "geometry"], axis=1
...
def read_geo_file(
    supernetwork_parameters, waterbody_parameters, compute_parameters, cpu_pool
):
...
class HYFeaturesNetwork(AbstractNetwork):
    ...
    def preprocess_network(self, flowpaths, nexus):
        ...
        self._flowpath_dict = dict(
            zip(self.dataframe.loc[mask].downstream, self.dataframe.loc[mask].key)
        )
        ...
        self._terminal_codes = self.terminal_codes | set(
            self.dataframe[~self.dataframe["downstream"].isin(self.dataframe.index)][
                "downstream"
            ].values
        )
    ...
    def preprocess_waterbodies(self, lakes, nexus):
        ...
        if not lakes.empty:
            ...
            self._waterbody_df["lake_id"] = self.waterbody_dataframe.lake_id.astype(
                float
            ).astype(int)
            self._waterbody_df = (
                self.waterbody_dataframe.set_index("lake_id")
                .drop_duplicates()
                .sort_index()
            )
            ...
            # to the lake_ids:
            duplicate_ids = list(
                set(self.waterbody_dataframe.index).intersection(
                    set(self.dataframe.index)
                )
            )
            ...
            update_dict = dict(
                self._duplicate_ids_df[["lake_id", "synthetic_ids"]].values
            )
            ...
            self._waterbody_df = self.waterbody_dataframe.rename(
                index=update_dict
            ).sort_index()
            ...
            wbody_conn = (
                self.dataframe[["waterbody"]].dropna().astype(int).reset_index()
            )
            ...
            if break_network_at_waterbodies:
                self._connections, self._link_lake_crosswalk = (
                    replace_waterbodies_connections(
            ...
            if lakeout:
                ...
                lat_lon_crs = (
                    lat_lon_crs.set_index("lake_id").drop_duplicates().sort_index()
                )
                lat_lon_crs = lat_lon_crs[
                    lat_lon_crs.index.isin(self.waterbody_dataframe.index)
                ]
            ...
            nexus["WBOut_id"] = (
                nexus["hl_uri"].str.extract(r"WBOut-(\d+)").astype(float)
            )
            great_lakes_df = nexus[
                nexus["WBOut_id"].isin([4800002, 4800004, 4800006, 4800007])
            ][["WBOut_id", "toid"]]
            if not great_lakes_df.empty:
                ...
                self._waterbody_df = pd.concat(
                    [self.waterbody_dataframe, gl_wbody_df]
                ).sort_index()
    ...
    def preprocess_data_assimilation(self, network):
        if not network.empty:
            ...
            gages_df[["type", "value"]] = gages_df.hl_uri.str.split(
                "-", expand=True, n=1
            )
            ...
            usgs_ind = (
                gages_df.value.str.isnumeric()
            )  # usgs gages used for streamflow DA
            ...
            # the hydrofabric.
            self._canadian_gage_link_df = pd.DataFrame(
                columns=["gages", "link"]
            ).set_index("link")
            ...
            lake_gage_hydroseq_df["lake_id"] = lake_gage_hydroseq_df["lake_id"].astype(
                int
            )
            ...
            if rfc_da:
                ...
                self._rfc_lake_gage_crosswalk = rfc_lake_gage_crosswalk[
                    rfc_lake_gage_crosswalk["rfc_lake_id"].isin(
                        self.waterbody_dataframe.index
                    )
    ...
    def build_qlateral_array(
        ...
        if qlat_input_folder:
            ...
            qlat_file_pattern_filter = self.forcing_parameters.get(
            ...
            if qlat_file_pattern_filter in ["nex-*", "cat-*"]:
                ...
                def process_file(f):
                    ...
                    try:
                        ...
                        raise ValueError(
                            f'Datetime column "{col_t0}" does not exist in nex-* files.'
                        )
    ...
    def bandaid(
        ...
        conn_df2 = (
            ...
            .drop("waterbody", axis=1)
            .merge(
                wbody_conn_df.rename(columns={"key": "downstream"}),
                on="downstream",
                how="left",
            )
        ...
        self._dataframe["waterbody"] = self._dataframe["waterbody"].replace(
            "1711354", "1710676"
        )
    ...
    def write_preprocessed_data(
        ...
        destination_folder = self.preprocessing_parameters.get(
    ...
    def read_preprocessed_data(
        ...
        preprocess_filepath = self.preprocessing_parameters.get(
            "preprocess_source_file", None
        )
        if preprocess_filepath:
            ...
            self._waterbody_type_specified = inputs.get(
                "waterbody_type_specified", None
            )
            self._link_lake_crosswalk = inputs.get("link_lake_crosswalk", None)
            self._gages = inputs.get("gages", None)
            self._usgs_lake_gage_crosswalk = inputs.get(
                "usgs_lake_gage_crosswalk", None
            )
            self._usace_lake_gage_crosswalk = inputs.get(
