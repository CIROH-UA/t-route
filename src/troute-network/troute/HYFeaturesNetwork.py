...
class HYFeaturesNetwork(AbstractNetwork):
    ...
    def build_qlateral_array(self, run,):
        ...
        if qlat_input_folder:
            ...
            if qlat_file_pattern_filter=="nex-*" or qlat_file_pattern_filter=="cat-*":
                def process_file(f):
                    if qlat_file_pattern_filter=="nex-*":
                        ...
                        cat_id = int((os.path.basename(f).split('-')[1].split('_')[0]).split('.')[0])
                    ...
                    df = df.T
                    df.index = [int((os.path.basename(f).split('-')[1].split('_')[0]).split('.')[0])]
