...
from troute.nhd_network import extract_connections, reverse_dict, reverse_network
...
def read_file_optimized(filepath):
    """Optimized file reader using csv module."""
    ...
    # Convert to DataFrame (still needed for compatibility)
    if data:
...
def get_ql_from_csv_optimized(filepath):
    """Optimized version of get_ql_from_csv using csv module."""
    ...
    with open(filepath, "r") as f:
        ...
        for row in reader:
            # Assuming the CSV has 'feature_id' column and timestamp columns
            feature_id = int(row.get("feature_id", row.get("id", 0)))

            for key, value in row.items():
                if key not in ["feature_id", "id"]:
                    try:
                        data_dict[feature_id][key] = float(value)
                    except (ValueError, TypeError):
                        data_dict[feature_id][key] = 0.0
...
def read_geopkg(file_path, compute_parameters, waterbody_parameters, cpu_pool):
    ...
    matched_layers = {
        key: find_layer_name(available_layers, pattern) for key, pattern in layer_patterns.items()
    }
    ...
    if any(
        [
            ...
            data_assimilation_parameters.get("reservoir_da", {}).get(
    ...
    if cpu_pool > 1:
        with Parallel(n_jobs=min(cpu_pool, len(layers_to_read))) as parallel:
            gpkg_list = parallel(
                delayed(read_layer)(matched_layers[layer]) for layer in layers_to_read
            )
    ...
    if not flowpath_attributes_df.empty and not flowpaths_df.empty:
        ...
        flowpaths = pd.merge(flowpaths_df, flowpath_attributes_df, on="id", how="inner")
...
class HYFeaturesNetwork(AbstractNetwork):
    ...
    __slots__ = ["_upstream_terminal", "_nexus_latlon", "_duplicate_ids_df", "_downstream_dict"]
    ...
    def preprocess_network(self, flowpaths, nexus):
        ...
        self._dataframe["key"] = (
            self._dataframe["key"].str.split("-").str[-1].astype(float).astype(int)
        )
        self._dataframe["downstream"] = (
            self._dataframe["downstream"].str.split("-").str[-1].astype(float).astype(int)
        )
    ...
    def crosswalk_nex_flowpath_poi(self, flowpaths, nexus):
        ...
        self._nexus_dict = (
            filtered_flowpaths.groupby("toid")["id"].apply(list).to_dict()
        )  ##{id: toid}
    ...
    def preprocess_waterbodies(self, lakes, nexus):
        ...
        if not lakes.empty:
            ...
            self._waterbody_df["lake_id"] = self.waterbody_dataframe.lake_id.astype(float).astype(
                int
            )
            self._waterbody_df = (
                self.waterbody_dataframe.set_index("lake_id").drop_duplicates().sort_index()
            )
            ...
            # to the lake_ids:
            duplicate_ids = list(
                set(self.waterbody_dataframe.index).intersection(set(self.dataframe.index))
            )
            ...
            tmp_wbody_conn = tmp_wbody_conn[
                tmp_wbody_conn["value"].isin(self.waterbody_dataframe.index)
            ]
            self._dataframe = (
            ...
            if lakeout:
                lat_lon_crs = lakes[["hl_link", "hl_reference", "geometry"]].rename(
                    columns={"hl_link": "lake_id"}
                )
            ...
            great_lakes_df = nexus[nexus["WBOut_id"].isin([4800002, 4800004, 4800006, 4800007])][
                ["WBOut_id", "toid"]
            ]
            if not great_lakes_df.empty:
                great_lakes_df["toid"] = (
                    great_lakes_df["toid"].str.extract(r"wb-(\d+)").astype(float)
                )
            ...
            self.data_assimilation_parameters["reservoir_da"]["reservoir_rfc_da"][
    ...
    def preprocess_data_assimilation(self, network):
        if not network.empty:
            ...
            # make 'id' an integer
            gages_df["id"] = (
                gages_df["id"].str.split("-", expand=True).loc[:, 1].astype(float).astype(int)
            )
            ...
            gages_df = (
                ...
                .join(
                    pd.DataFrame().from_dict(
                        self.waterbody_connections, orient="index", columns=["lake_id"]
                    )
                )
            )
            ...
            lake_gage_hydroseq_df = gages_df[~gages_df["lake_id"].isnull()][
                ["lake_id", "value", "hydroseq"]
            ].rename(columns={"value": "gages"})
            ...
            lake_gage_hydroseq_df = (
                lake_gage_hydroseq_df.groupby(["lake_id", "gages"])
                .max("hydroseq")
                .reset_index()
                .set_index("lake_id")
            )
            ...
            self._usgs_lake_gage_crosswalk = (
                ...
                .merge(
                    lake_gage_hydroseq_df.rename_axis("usgs_lake_id").rename(
                        columns={"gages": "usgs_gage_id"}
                    ),
            ...
            self._usace_lake_gage_crosswalk = (
                ...
                .merge(
                    lake_gage_hydroseq_df.rename_axis("usace_lake_id").rename(
                        columns={"gages": "usace_gage_id"}
                    ),
            ...
            if usace_da:
                self._waterbody_types_df.loc[
                    self._usace_lake_gage_crosswalk.index, "reservoir_type"
                ] = 3
            if usgs_da:
                self._waterbody_types_df.loc[
                    self._usgs_lake_gage_crosswalk.index, "reservoir_type"
                ] = 2
            if rfc_da:
                ...
                self._waterbody_types_df.loc[
                    self._rfc_lake_gage_crosswalk.index, "reservoir_type"
                ] = 4
            else:
    ...
    def build_qlateral_array(self, run):
        """Optimized version of build_qlateral_array using built-in csv reader."""
        ...
        if qlat_input_folder:
            ...
            if qlat_file_pattern_filter in ["nex-*", "cat-*"]:
                ...
                def process_file_fast(f):
                    ...
                    with open(f, "r") as csvfile:
                        if qlat_file_pattern_filter == "nex-*":
                            ...
                            # For cat-* files: has headers, use Time and Q_OUT columns
                            reader = csv.DictReader(csvfile)
                            cat_id = f.stem
                            area = areas.get(cat_id, 1.0)  # Default to 1.0 if not found
                            scaling_factor = (area * 1000000) / 3600

                            for row in reader:
                                ts = datetime.fromisoformat(row["Time"]).strftime("%Y%m%d%H%M")
                                timestamps.append(ts)
                                # Apply scaling directly during reading
                                values.append(float(row["Q_OUT"]) * scaling_factor)

                    # Return as dictionary for easy DataFrame construction
                    return {"feature_id": feature_id, "data": dict(zip(timestamps, values))}

                # Process files in parallel
                with Parallel(n_jobs=-1) as p:
                    results = p(delayed(process_file_fast)(f) for f in qlat_files)

                # Build DataFrame more efficiently
                # First, collect all unique timestamps
                all_timestamps = set()
                for r in results:
                    all_timestamps.update(r["data"].keys())
                all_timestamps = sorted(all_timestamps)

                # Build data matrix
                feature_ids = [r["feature_id"] for r in results]
                data_matrix = np.full(
                    (len(feature_ids), len(all_timestamps)), np.nan, dtype=np.float32
                )

                for i, r in enumerate(results):
                    for j, ts in enumerate(all_timestamps):
                        if ts in r["data"]:
                            data_matrix[i, j] = r["data"][ts]

                # Create DataFrame
                lateralflows_df = pd.DataFrame(
                    data_matrix,
                    index=pd.Index(feature_ids, name="feature_id"),
                    columns=all_timestamps,
                )

            else:
                # Original logic for other file patterns, but optimized
                dfs = []

                for f in qlat_files:
                    # Use optimized CSV reading for standard files
                    df = read_file_optimized(f)

                    # Process feature_id column
                    if "feature_id" in df.columns:
                        df["feature_id"] = df["feature_id"].apply(
                            lambda x: int(str(x).removeprefix("nex-"))
                            if str(x).startswith("nex")
                            else int(x)
                        )
                        ...
                        if not df["feature_id"].is_unique:
                            duplicates = df.loc[
                                df["feature_id"].duplicated(), "feature_id"
                            ].to_list()
                            raise AssertionError(
                                f"'feature_id's must be unique. '{f!s}' contains duplicate 'feature_id's: {duplicates}"
                            )
                ...
                lateralflows_df = pd.concat(dfs, axis=1) if dfs else pd.DataFrame()

            qlats_df = lateralflows_df

            # Filter by segment_index
            if not qlats_df.empty and hasattr(self, "segment_index"):
                # Use intersection for faster filtering
                valid_indices = qlats_df.index.intersection(self.segment_index)
                qlats_df = qlats_df.loc[valid_indices]

                # Sort index for better performance in subsequent operations
                qlats_df = qlats_df.sort_index()

        elif qlat_input_file:
            # Use optimized CSV reading
            qlats_df = get_ql_from_csv_optimized(qlat_input_file)

        else:
            # Create constant value dataframe
        ...
        if len(qlats_df.columns) > max_col:
            qlats_df = qlats_df.iloc[:, :max_col]

        # Final filtering by segment_index
        if not self.segment_index.empty and not qlats_df.empty:
            valid_indices = qlats_df.index.intersection(self.segment_index)
            qlats_df = qlats_df.loc[valid_indices]

        self._qlateral = qlats_df

    # Additional optimization: Batch processing for very large datasets
    def process_files_in_batches(files, batch_size=100, process_func=None):
        ...
        for i in range(0, len(files), batch_size):
            ...
            with Parallel(n_jobs=-1) as p:
                batch_results = p(delayed(process_func)(f) for f in batch)
    ...
    for n in connections:
        if n in waterbodies:
            ...
            if len(outgoing) >= 1:
                ...
                subset_dict = {
                    key: value for key, value in connections.items() if key in wbody_nodes
                }
